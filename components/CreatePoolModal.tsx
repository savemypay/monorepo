...
import { X, Plus, Trash2, Upload, Image as ImageIcon, Calendar, Layers, Loader2, AlertCircle, CheckCircle2 } from 'lucide-react';
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh]">
        <div className="px-6 py-5 border-b border-gray-100 flex justify-between items-center bg-white rounded-t-2xl">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">{isSubmitted ? 'Submission Status' : 'Define your bulk offer parameters'}</p>
          </div>
        ...
        {isSubmitted ? (
          <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
            ...
            <p className="text-sm text-gray-600 mt-2 max-w-md">{successMessage}</p>
            <button
              onClick={onClose}
              className="mt-6 px-8 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl transition-colors"
            >
              OK
            </button>
          </div>
                     ...
                     {tiers.map((tier, index) => (
                       <div key={index} className="grid grid-cols-12 gap-2 px-4 py-3 items-center hover:bg-white transition-colors">
                         ...
                         <div className="col-span-5">
                           <input value={tier.quantity} onChange={(e) => updateTier(index, 'quantity', e.target.value)} type="text" inputMode="numeric" pattern="[0-9]*" className="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm outline-none focus:border-blue-500" placeholder="Qty" />
                         </div>
                         <div className="col-span-5">
                           <input value={tier.discount} onChange={(e) => updateTier(index, 'discount', e.target.value)} type="text" inputMode="numeric" pattern="[0-9]*" className="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm outline-none focus:border-blue-500" placeholder="%" />
                         </div>
