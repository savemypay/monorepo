...
export default function Home2HowItWorks() {
  return (
    <section id="how" className="px-6 py-12 md:py-20 bg-white">
      <div className="max-w-7xl mx-auto">
        <div className="text-center">
          <span className={`inline-block bg-[#fdf3dc] text-[#e8a830] px-3 py-1 rounded-[20px] text-[11px] font-bold tracking-[1px] uppercase mb-5 ${REVEAL}`} data-reveal>
            Simple Process
          </span>
          <h2 className={`text-[28px] md:text-[36px] lg:text-[54px] leading-[1.15] font-extrabold mb-4 ${REVEAL}`} data-reveal style={{ transitionDelay: "100ms" }}>
            From Interest to <em className="not-italic text-[#1b3a6b]">Savings in 4 Steps</em>
          </h2>
          <p className={`text-lg text-[#5a7090] leading-[1.75] max-w-xl mx-auto ${REVEAL}`} data-reveal style={{ transitionDelay: "200ms" }}>
        ...
        <div className="relative mt-15 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-7">
          <div className="hidden lg:block absolute top-7 left-[0%] right-[9%] h-0.5 bg-[linear-gradient(90deg,#e8a830,#6ec6c0,rgba(110,198,192,0.1))]" />
          {HOW_STEPS.map((step, index) => (
            ...
            >
              ...
              <h4 className="text-lg font-bold text-[#1b3a6b] mb-2">{step.title}</h4>
              <p className="text-sm text-[#5a7090] leading-[1.6]">{step.description}</p>
            </article>
          ))}
        </div>
      </div>
