...
function CategoryCard({ item, index }: { item: (typeof CATEGORIES)[number]; index: number }) {
  return (
    <article
      className={`bg-white pb-5 rounded-2xl border border-[#dfe6ef] hover:border-[#e8a830] hover:shadow-[0_18px_44px_rgba(27,58,107,0.12)] transition-all relative overflow-hidden group ${REVEAL}`}
      data-reveal
    ...
    >
      <div className="relative h-44 w-full overflow-hidden">
        <Image
          ...
          alt={item.title}
          fill
          className="object-cover transition-transform duration-500 group-hover:scale-105 px-5 py-5 rounded-t-2xl"
        />
