...
const WHY_STATS = [
  ...
  { icon: "/assets/Icon/Asset 38.svg", value: "1 Week", label: "Average deal close" },
  { icon: "/assets/Icon/Asset 39.svg", value: "100%", label: "Secure payments" },
  { icon: "/assets/Icon/Asset 40.svg", value: "Verified", label: "Every vendor, always" },
];
      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[5fr_7fr] gap-3 items-stretch">
          <div className="flex flex-col gap-3">
            <article
              className={`group relative overflow-hidden rounded-[20px] p-8 md:p-9 text-white bg-[#0C111A] border border-[rgba(110,198,192,0.28)] transition-all duration-300 hover:-translate-y-0.5 ${REVEAL}`}
              data-reveal
              style={{ transitionDelay: "100ms" }}
            >
              ...
              <div className="relative z-[2]">
                <div className="mb-4 flex h-[52px] w-[52px] items-center justify-center rounded-[14px] border border-[rgba(110,198,192,0.32)] bg-[rgba(110,198,192,0.18)] text-[1.5rem] transition-transform duration-300 group-hover:rotate-[8deg] group-hover:scale-105">
                  <Image src={heroItem.icon} alt={heroItem.title} width={32} height={32}/>
                </div>
                ...
                <h3 className="text-[30px] md:text-[34px] leading-none font mb-3">{heroItem.title}</h3>
                <p className="text-base leading-[1.75] text-white/75">{heroItem.description}</p>
              </div>
            </article>
            ...
            >
              ...
              <div className="relative z-[2]">
                <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-xl border border-[rgba(110,198,192,0.25)] bg-[rgba(110,198,192,0.12)] text-[1.3rem] transition-transform duration-300 group-hover:rotate-[8deg] group-hover:scale-105">
                  <Image src={supportItem.icon} alt={supportItem.title} width={24} height={24}/>
                </div>
                <h4 className="text-lg md:text-xl leading-tight font-bold text-[#1b3a6b] mb-2">{supportItem.title}</h4>
                <p className="text-base leading-[1.75] text-[#5a7090]">{supportItem.description}</p>
              </div>
            </article>
          </div>
            ...
            {tiles.map((item, index) => (
              ...
              >
                ...
                <div className="relative z-[2]">
                  ...
                  <h4 className="text-lg md:text-xl leading-tight font-bold text-[#1b3a6b] mb-2">{item.title}</h4>
                  <p className="text-base leading-[1.75] text-[#5a7090]">{item.description}</p>
                </div>
              </article>
            ))}
          </div>
        ...
        <div
          className={`relative mt-10 grid grid-cols-2 lg:grid-cols-4 overflow-hidden rounded-[20px] border border-[rgba(255,255,255,0.06)] bg-[#0C111A] ${REVEAL}`}
          data-reveal
        ...
        >
          ...
          {WHY_STATS.map((stat, index) => (
            <article
              key={stat.value}
              className={`flex items-center gap-4 md:gap-5 px-5 py-5 transition-colors hover:bg-[rgba(110,198,192,0.1)] ${
            ...
            >
              <div className="text-[1.2rem] shrink-0">
                <Image src={stat.icon} alt={stat.value} width={30} height={30}/>
              </div>
              <div>
                <p className="text-[#F2B705] text-lg md:text-xl leading-none font-bold">{stat.value}</p>
                <p className="mt-1 text-sm uppercase tracking-[1.2px] text-white/55 font-semibold">{stat.label}</p>
              </div>
            </article>
          ))}
        </div>
