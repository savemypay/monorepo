...
class NotificationService:
    ...
    def list_actor_notifications(
        ...
        *,
        ...
        return (
            ...
            .filter(
                NotificationDelivery.actor_type == actor_type,
                NotificationDelivery.actor_id == actor_id,
                NotificationDelivery.channel == "inbox",
            )
    ...
    def send_push_to_actor(
        ...
        *,
        ...
        actor_type = self._validate_actor(actor_type=actor_type, actor_id=actor_id)
        self._create_actor_inbox_delivery(
    ...
    def send_push_to_installation(
        ...
        *,
        ...
        if not bool(installation.notifications_enabled):
            return []
        if installation.actor_type is not None and installation.actor_id is not None:
            try:
                self._create_actor_inbox_delivery(
                    actor_type=installation.actor_type,
                    actor_id=int(installation.actor_id),
                    category=category,
                    message=message,
                    source="push_installation",
                )
            except ValueError:
                logger.warning(
                    "Skipped actor inbox notification for installation_id=%s invalid_actor_type=%s invalid_actor_id=%s",
    ...
    def _create_actor_inbox_delivery(
        self,
        *,
        actor_type: str,
        actor_id: int,
        ...
        delivery = NotificationDelivery(
            ...
            sent_at=now,
            delivered_at=now,
            created_by=f"notification_{source}_inbox",
        )
        self.db.add(delivery)
        return delivery
