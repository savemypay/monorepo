...
def _display_period_start(bucket_start: date, date_from: date) -> date:
    return max(bucket_start, date_from)


def _get_onboarding_query_parts(role: UserRole):
    if role == "vendor":
        return VendorAccount.id, VendorAccount.created_at, (VendorAccount.role == "vendor",)
    return User.id, User.created_at, (User.role == "customer",)
...
def get_user_onboarding_trend(
    ...
    *,
    ...
    id_column, created_at_column, base_filters = _get_onboarding_query_parts(role)

    base_count = (
        db.query(func.count(id_column))
        .filter(
            *base_filters,
            created_at_column < range_start,
        )
        .scalar()
        or 0
    )
    ...
    rows = (
        db.query(created_at_column)
        .filter(
            *base_filters,
            created_at_column >= range_start,
            created_at_column < range_end_exclusive,
        )
        .all()
    )
    ...
    while cursor <= date_to:
        ...
        trend.append(
            {
                "period_start": _display_period_start(cursor, date_from).isoformat(),
