...
def _get_latest_payment_for_deal(
    db: Session,
    *,
    customer_ref: str,
    deal_ref: str,
...
def _upsert_token_payment_metadata(
    db: Session,
    *,
    ad_id: int,
    customer_id: int,
    ...
    existing = (
        db.execute(
            select(TokenPaymentMetadata).where(
                TokenPaymentMetadata.ad_id == ad_id,
                TokenPaymentMetadata.customer_id == customer_id,
                TokenPaymentMetadata.vendor_id == vendor_id,
            )
        )
        .scalars()
        .first()
...
def initiate_token_payment(
    ...
    *,
    ...
    latest_payment = _get_latest_payment_for_deal(
        db,
        customer_ref=customer_ref,
        deal_ref=deal_ref,
    )
    if latest_payment is None:
        _upsert_token_payment_metadata(
            db,
            ad_id=ad.id,
            customer_id=customer_id,
    ...
    elif latest_payment.status in (PaymentStatus.FAILED, PaymentStatus.CANCELED):
        _upsert_token_payment_metadata(
            db,
            ad_id=ad.id,
            customer_id=customer_id,
