...
def _apply_referral_reward(db: Session, user: User, referrer: User) -> None:
    ...
    reward_points = max(REFERRAL_REWARD_POINTS, 0)
    if reward_points <= 0:
        return

    result = RewardService(db).grant_reward(
        user_id=referrer.id,
        event_type=REFERRAL_REGISTER_EVENT,
        source_type="referred_user",
        source_id=user.id,
    ...
    reward = ReferralReward(
        ...
        payment_id=None,
        event_type=REFERRAL_REGISTER_EVENT,
        reward_amount=result.transaction.points,
        status="credited",
        created_by="referral",
    )
    db.add(reward)
...
def _send_login_push_notification(db: Session, *, actor_type: str, actor_id: int) -> None:
    try:
        deliveries = NotificationService(db).send_push_to_actor(
        ...
        logger.info(
            "Login push dispatch actor_type=%s actor_id=%s deliveries=%s",
            actor_type,
            actor_id,
            len(deliveries),
        )
    except Exception:
        logger.exception("Login push dispatch failed actor_type=%s actor_id=%s", actor_type, actor_id)


def _apply_daily_login_reward(db: Session, *, user_id: int) -> None:
    if DAILY_LOGIN_STREAK_POINTS <= 0:
        return

    try:
        result = RewardService(db).grant_daily_login_reward(
            user_id=user_id,
            points=DAILY_LOGIN_STREAK_POINTS,
        )
        logger.info(
            "Daily login reward user_id=%s rewarded=%s streak=%s longest_streak=%s points=%s",
            user_id,
            result.rewarded,
        ...
        logger.exception("Daily login reward failed user_id=%s", user_id)
...
def verify_otp(db: Session, audience: Audience, payload: OTPVerifyRequest) -> dict:
    ...
    if audience == "vendor":
        ...
        _send_login_push_notification(db, actor_type="vendor", actor_id=vendor.id)
        return tokens
    else:
        user, is_new = _ensure_user_record(db, payload, audience)
        tokens = _issue_tokens_customer(db, user, is_new)
        _apply_daily_login_reward(db, user_id=user.id)
        _send_login_push_notification(db, actor_type="customer", actor_id=user.id)
        return tokens
