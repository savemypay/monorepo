...
def _serialize_actor_notification(delivery: NotificationDelivery) -> dict:
    parsed_data: dict[str, str] = {}
    if delivery.data_json:
        try:
            raw_data = json.loads(delivery.data_json)
            if isinstance(raw_data, dict):
                parsed_data = {str(key): str(value) for key, value in raw_data.items()}
        except (TypeError, ValueError):
            parsed_data = {}

    return {
        "id": delivery.id,
        "actor_type": str(delivery.actor_type),
        "actor_id": int(delivery.actor_id),
...
@router.get("/me", status_code=status.HTTP_200_OK, response_model=NotificationInboxResponse)
def list_actor_notifications(
    ...
    rows = service.list_actor_notifications(actor_type=actor_type, actor_id=actor_id, limit=limit, offset=offset)
    data = [_serialize_actor_notification(row) for row in rows]
    return success_response(message="Notifications fetched", data=data)
